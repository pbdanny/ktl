...
# 0101235 sec (does not exist) SALAD -> 0171136 (became WS SALAD)
# 0140071 sec (does not exist) FROZEN -> 0140 (became department) use value of 0140 (keep both)
# 0233707 sec (does not exist) CP-CONFECTIONERY -> CP-Snack+Confectionery 0266353
...
# replaced_col = list(map(lambda x: '_'.join(replacements.get(word, word) for word in x.split('_')), old_col))
...
# DBTITLE 1,FINAL SELECTED FEATURES SHOULD MATCH
selected_features = [
...
# DBTITLE 1,Filter Only Selected Features
...
# DBTITLE 1,Check Missing Features
...
# 0204 dep (in exlcude list)
# 0204009 sec (in exclude list)
# 0233 dep (does not exist) CP DG FOOD (not in CASE)
# 1046020 sec (does not exist) CANCEL CLEANING (not in CASE)
# 0101235 sec (does not exist) SALAD -> 0171136 (became WS SALAD)
# 0140071 sec (does not exist) FROZEN -> 0140 (became department)
# 0314080 sec (does not exist) CS2-ACCESSORIES (not in CASE)
# 0233707 sec (does not exist) CP-CONFECTIONERY -> CP-Snack+Confectionery 0266353

# COMMAND ----------

list(filtered_dic.keys())

# COMMAND ----------

features_df = df.select(list(filtered_dic.values()))
