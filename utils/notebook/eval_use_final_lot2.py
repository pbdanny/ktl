...
feedback1_azay.groupBy("lot").count().display()
...
feedback_cigna.select("lead_type").distinct().display()
...
cust_lot = lot2.filter(F.col("InsurerName")=="AAC").join(cust, "household_id", "inner")
feedback_az_eval = feedback_azay.filter(F.col("lead_type").isin(lead_type_lst))

# COMMAND ----------

feedback_az_eval.groupBy("Contact_Status").count().display()

# COMMAND ----------

feedback_az_eval.filter(F.col("Contact_Status")=="Uncontactable").groupBy("Reason_code").count().display()

# COMMAND ----------

cust_lot.groupBy("LeadType").count().withColumnRenamed("count", "Total").join(
    cust_lot.groupBy("LeadType").pivot("CashorCreditCard").count(), on="LeadType"
).withColumnRenamed("LeadType", "# of lead").display()

# COMMAND ----------

lead_shared_cnt = cust_lot.count()
...
cust_lot.groupBy("LeadType").pivot("CashorCreditCard").count().display()
...
def get_funnel(group, order_col, cust=cust_lot, feedback=feedback_az_eval):
    ...
    s = cust.filter((F.col(group) != "Unclassified") & (F.col(group) != "NA"))
...
cust_lot.select("lifestage_seg_name").distinct().display()
...
cust_lot.filter((F.col(group) != "Unclassified") & (F.col(group) != "NA")).groupBy("lifestage_seg_name").count().display()
...
get_funnel(
    group,
    order_col=order_col,
    cust=cust_lot.filter(F.col("LeadType") == "hot"),
    feedback=feedback_az_eval.filter(F.col("lead_type") == "Hot"),
)
...
get_funnel(
    group,
    order_col=order_col,
    cust=cust_lot.filter(F.col("LeadType") == "normal"),
    feedback=feedback_az_eval.filter(F.col("lead_type") == "Normal"),
)
...
eval_pipe2(feedback_azay)

# COMMAND ----------

# MAGIC %md
# MAGIC ### index performance

# COMMAND ----------

for elum in index_lists:
    _index = index_performance(cust_lot, feedback_az_eval, elum)
    _index.sort("index",ascending=False).display()

# COMMAND ----------

_index = index_performance(
    cust_lot.filter(F.col("pref_store_region") == "West"),
    feedback_az_eval,
    "pref_store_id",
)
...
cust_lot = lot2.filter(F.col("InsurerName")=="CIG").join(cust, "household_id", "inner")
...
(
    cust_lot.groupBy("LeadType")
    .pivot("CashorCreditCard")
    .count()
    .withColumnRenamed("LeadType", "lead_type")
    .withColumn("# Lead", F.lit("Send"))
    .unionByName(
        feedback_cigna_eval.groupBy("lead_type")
        .pivot("lead_shop_payment_type")
        .count()
        .withColumn("# Lead", F.lit("Used"))
    )
    .unionByName(
        contactable.groupBy("lead_type")
        .pivot("lead_shop_payment_type")
        .count()
        .withColumn("# Lead", F.lit("Contactable"))
    )
    .unionByName(
        bought.groupBy("lead_type")
        .pivot("lead_shop_payment_type")
        .count()
        .withColumn("# Lead", F.lit("Bought"))
    )
    .unionByName(
        payment.groupBy("lead_type")
        .pivot("lead_shop_payment_type")
        .count()
        .withColumn("# Lead", F.lit("Payment"))
    )
).select("# Lead", "lead_type", "Cash", "Credit").display()

# COMMAND ----------

feedback_cigna_eval.groupBy("lead_type").pivot("model_score_bucket").count().withColumn(
    "# Lead", F.lit("Used")
).unionByName(
    contactable.groupBy("lead_type")
    .pivot("model_score_bucket")
    .count()
    .withColumn("# Lead", F.lit("Contactable"))
).unionByName(
    bought.groupBy("lead_type")
...
def get_funnel(group, order_col, cust, feedback):
    cust_feedback = feedback.select(
        F.col("dib_cust_code").alias("household_id"),
        "contact_status",
        "bought_status",
    ...
    p = b.filter(F.col("Payment_Type").isNotNull())
    s.groupby().pivot(group).count().withColumn("# Lead", F.lit("Send")).unionByName(
        u.groupby().pivot(group).count().withColumn("# Lead", F.lit("Used")), True
    ).unionByName(
        c.groupby().pivot(group).count().withColumn("# Lead", F.lit("Contactable")),
        True,
    ).unionByName(
        b.groupby().pivot(group).count().withColumn("# Lead", F.lit("Bought")), True
    ).unionByName(
        p.groupby().pivot(group).count().withColumn("# Lead", F.lit("Payment")), True
    ).select(
        *order_col
    ).fillna(0).display()
...
get_funnel(group, order_col=order_col, cust=cust_lot, feedback=feedback_cigna_eval)
...
# cust_lot.select(group).distinct().display()
get_funnel(group, order_col=order_col, cust=cust_lot, feedback=feedback_cigna_eval)
...
for elum in index_lists:
    _index = index_performance(cust_lot, feedback_cigna_eval, elum)
    _index.sort("index",ascending=False).display()

# COMMAND ----------

_index = index_performance(
    cust_lot.filter(F.col("pref_store_region") == "Central"),
    feedback_cigna_eval,
    "pref_store_id",
)
_index.join(dim_store, F.col("pref_store_id" ) == F.col("store_id"), "left").sort(
