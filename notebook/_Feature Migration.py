...
def appendDropDupes(list1, list2):
...
week = {'lowerBound':202313, 'upperBound':202314}
country = 'th'
...
# DBTITLE 1,Create Sampled Data (dont need to run anymore) 
...
item_df = sqlContext.table('tdm.v_transaction_item').select(['transaction_uid','store_id','date_id','week_id','tran_datetime','upc_id','customer_id','net_spend_amt','cc_flag','count_qty','product_qty','measured_qty','discount_amt','source'])\
                                                    ...
                                                    .dropDuplicates()
                                                    # .filter(F.col('week_id').between(week['lowerBound'], week['upperBound']))\
...
data_df = data_df.sample(0.00005, 42)

data_df.write.saveAsTable("tdm_seg.kritawatkrai_th_sample_year_full_data_jun_29_tmp")


# COMMAND ----------

# sample data full year
data_df = spark.table("tdm_seg.kritawatkrai_th_sample_year_full_data_jun_29_tmp")
...
max_week_december = time_of_day_df.filter((F.col("month_id") % 100) == 12).filter(F.col("week_id").startswith(F.col("month_id").substr(1, 4))) \
                     .agg(max(F.col("week_id")).alias("max_week_december")).collect()[0]["max_week_december"]
...
last_sat = date_df.filter(F.col('weekday_nbr') == 6).groupBy('month_id').agg(max('day_in_month_nbr').alias('day_in_month_nbr'))\
...
r = flagged_df.withColumn('end_date', lit(timeframe_end))\
          ...
          .withColumn('start_month_date', date_sub(trunc(F.col('date_id'), 'month'), 0))\
...
# removing traders 
trader_df = sqlContext.table('tdm_seg.trader_subseg_party2_master')
max_quarter_id = 0
...
trader_df = sqlContext.table('tdm_seg.trader_subseg_party2_master').filter(F.col('quarter_id') == max_quarter_id)
...
flag_df.write.mode("overwrite").saveAsTable("tdm_seg.kritawatkrai_th_sample_year_full_data_jun_29_flag_tmp")

# COMMAND ----------

# DBTITLE 1,Load Table Back
flag_df = spark.table("tdm_seg.kritawatkrai_th_sample_year_full_data_jun_29_flag_tmp")


# COMMAND ----------

flag_df.select('time_of_day').distinct().display()
...
# check no dummy
flag_df.filter(F.col('household_id') == -1).display()

# COMMAND ----------

flag_df.select('household_id').distinct().count()
# 47498 (no dummy)

# COMMAND ----------


flag_df.select(sum('net_spend_amt')).display()
# 4058619.839441

# COMMAND ----------

flag_df.groupBy('household_id','transaction_uid', 'upc_id', 'date_id', 'store_id').count().filter("count > 1").display()


# COMMAND ----------

# DBTITLE 1,Adding Promo
start_promoweek = date_dim.filter(F.col('date_id').between(timeframe_start, timeframe_end)).agg(min('promoweek_id')).collect()[0][0]
end_promoweek = date_dim.filter(F.col('date_id').between(timeframe_start, timeframe_end)).agg(max('promoweek_id')).collect()[0][0]
...
df_promo = spark.table('tdm.tdm_promo_history_rms19').filter(F.col('active_flag') == 'Y')\
                                                     .filter(~(F.col('change_type').isNull()))

df_promozone = spark.table('tdm.promo_zone_master')


df_prod = sqlContext.table('tdm.v_prod_dim_c').filter(F.col('division_id').isin(product_division))\
                                                 .filter(F.col('country') == country)

df_trans_item = sqlContext.table('tdm.v_transaction_item').filter(F.col('week_id').between(start_week, end_week))\
                                                    .filter(F.col('date_id').between(timeframe_start,timeframe_end))\
                                                    .filter(F.col('country') == country)\
                                                    .where((F.col('net_spend_amt')>0)&(F.col('product_qty')>0)&(F.col('date_id').isNotNull()))\
                                                    .filter(F.col('cc_flag') == 'cc')\
...
df_store = sqlContext.table('tdm.v_store_dim_c').filter(F.col('format_id').isin(store_format))\
                                                .filter(F.col('country') == country)\
                                                .withColumn('format_name', when(F.col('format_id').isin([1,2,3]), 'Hypermarket')\
                                                                             .when(F.col('format_id') == 4, 'Supermarket')\
                                                                             .when(F.col('format_id') == 5, 'Mini Supermarket')\
...
condition = (flag_df['date_id'].between(df_promo_with_stores['promo_start_date'], df_promo_with_stores['promo_end_date'])) & (flag_df['upc_id'] == df_promo_with_stores['upc_id']) & (flag_df['store_id'] == df_promo_with_stores['store_id'])

promo_df = flag_df.join(df_promo_with_stores, condition, 'left')

promo_df = promo_df.drop(df_promo_with_stores['upc_id'], df_promo_with_stores['date_id'], df_promo_with_stores['store_id'], df_promo_with_stores['source'], df_promo_with_stores['promoweek_id'])

# check = test_promo_df.select('household_id', 'transaction_uid', full_flag_df['upc_id'], full_flag_df['store_id'], full_flag_df['date_id'], 'promo_start_date', 'promo_end_date')
...
# check.display()
# 49094 with disinct
# 533713 without distinct


# COMMAND ----------

# DBTITLE 1,Save Table 
promo_df.write.mode("overwrite").saveAsTable("tdm_seg.kritawatkrai_th_sample_year_full_data_jun_29_w_promo_tmp")

# COMMAND ----------

# DBTITLE 1,Load Table Back
promo_df = spark.table("tdm_seg.kritawatkrai_th_sample_year_full_data_jun_29_w_promo_tmp")


# COMMAND ----------

flag_promo_df = promo_df.withColumn('promo_gap', datediff(F.col('promo_end_date'), col('promo_start_date')))\
               .withColumn("row_number", row_number().over(Window.partitionBy('household_id', 'transaction_uid', 'upc_id', 'store_id', 'date_id')
                                                     .orderBy(F.col('promo_start_date'), col('promo_gap').desc())))

flag_promo_df = flag_promo_df.filter(F.col('row_number') == 1)
...
dep_exclude = ['1_36','1_92','13_25','13_32','2_4']
sec_exclude = ['3_7_130', '3_7_131', '3_8_132', '3_9_81', '10_43_34', '3_14_78', '13_6_205', '13_67_364',
    ...
    '13_25_250', '13_25_251', '13_67_359', '2_66_350', '4_10_84', '10_11_72', '4_56_111', '10_46_549',
...
flag_promo_df = flag_promo_df.unionByName(div_cust, allowMissingColumns=True)\
                           ...
                           .withColumn('discount_reason_code', when(~(F.col('promo_gap').isNull()), 'P')\
                                                              .when((F.col('promo_gap').isNull()) & (F.col('discount_amt') > 0), 'M')\
                                                              .otherwise('NONE'))\
                           .withColumn('promo_flag', when(F.col('discount_reason_code') == 'P', 'PROMO')\
                                                    .when(F.col('discount_reason_code') == 'M', 'MARK_DOWN')\
...
flag_promo_df.select('discount_reason_code', 'promo_flag').display()

# COMMAND ----------

display(flag_promo_df.groupBy('household_id','transaction_uid', 'upc_id', 'date_id', 'store_id').count().filter("count > 1"))

# COMMAND ----------

flag_promo_df.count() - flag_df.count()

# COMMAND ----------

flag_promo_df.select(sum('net_spend_amt')).display()
...
# DBTITLE 1,Save Table 
flag_promo_df.write.mode("overwrite").saveAsTable("tdm_seg.kritawatkrai_th_sample_year_full_data_jun_29_w_promo_w_dummy_tmp")

# COMMAND ----------

# DBTITLE 1,Load Table
full_flag_df = spark.table('tdm_seg.kritawatkrai_th_sample_year_full_data_jun_29_w_promo_w_dummy_tmp')

# COMMAND ----------

full_flag_df.filter(F.col('division_id').isNull()).display()

# COMMAND ----------

# DBTITLE 1,Check Partition Column
# MAGIC %sql
# MAGIC describe extended  stdm.transtender;
...
total_df.filter(F.col('household_id') == 102111060000215810).display()
...
div_df = full_flag_df.filter(~(F.col('division_id').isNull()))\
...
dep_df = full_flag_df.filter(~(F.col('grouped_department_code').isNull()))\
...
# pivoted_dep_df = pivoted_dep_df.withColumns 
...
# DBTITLE 1,Section
sec_df = full_flag_df.filter(~(F.col('grouped_section_code').isNull()))\
...
half_agg_df = total_df.join(pivoted_dep_df, on='household_id', how='left')\
                       ...
                       .join(pivoted_div_df, on='household_id', how='left')\
...
# DBTITLE 1,Save Table
half_agg_df.write.mode("overwrite").saveAsTable("tdm_seg.kritawatkrai_th_sample_year_full_agg_data_jun_29_tmp")
...
monthly_df = monthly_df.groupBy('household_id').agg(
        ...
        round(min(coalesce(F.col("Spend"), lit(0))), 2).alias("MIN_SPEND_MNTH"),
        round(max(coalesce(F.col("Spend"), lit(0))), 2).alias("MAX_SPEND_MNTH"),
        ...
        round(min(coalesce(F.col("Visits"), lit(0))), 2).alias("MIN_VISITS_MNTH"),
        round(max(coalesce(F.col("Visits"), lit(0))), 2).alias("MAX_VISITS_MNTH"),
        round(avg(coalesce(F.col("Units"), lit(0))), 2).alias("AVG_UNITS_MNTH"),
        round(stddev(coalesce(F.col("Units"), lit(0))), 2).alias("SD_UNITS_MNTH"),
        round(min(coalesce(F.col("Units"), lit(0))), 2).alias("MIN_UNITS_MNTH"),
        round(max(coalesce(F.col("Units"), lit(0))), 2).alias("MAX_UNITS_MNTH")
).fillna(0)
...
pivot_columns = appendDropDupes(pivot_columns, region_list)
...
full_flag_df.select('week_id', 'period_id', 'quarter_id').distinct().display()

# COMMAND ----------

# DBTITLE 1,Online Offline
channel_df = full_flag_df.groupBy('household_id','channel_flag')\
                       .agg(sum('net_spend_amt').alias('Spend'), \
                        countDistinct('unique_transaction_uid').alias('Visits'), \
                        sum('unit').alias('Units'))
                                               
channel_df = channel_df.join(total_df, on='household_id', how='inner')

channel_df = channel_df.withColumn('SPV', when((F.col('Visits').isNull()) | (F.col('Visits') == 0), 0)\
                                 .otherwise(F.col('Spend') / col('Visits')))\
               .withColumn('UPV', when((F.col('Visits').isNull()) | (F.col('Visits') == 0), 0)\
                                 .otherwise(F.col('Units') / col('Visits')))\
               .withColumn('SPU', when((F.col('Units').isNull()) | (F.col('Units') == 0), 0)\
...
pivot_columns = channel_df.select("channel_flag").distinct().rdd.flatMap(lambda x: x).collect()
pivoted_channel_df = channel_df.groupBy("household_id").pivot("channel_flag", pivot_columns).agg(
    first("Spend").alias("Spend"),
    first("Visits").alias("Visits"),
    first("Units").alias("Units"),
    first("SPV").alias("SPV"),
...
for c in pivot_columns:
    pivoted_channel_df = pivoted_channel_df.withColumnRenamed(c +"_PCT_Spend", "PCT_" + c + "_SPEND")\
                                   .withColumnRenamed(c +"_PCT_Visits", "PCT_" + c + "_VISITS")\
                                   .withColumnRenamed(c +"_PCT_Units", "PCT_" + c + "_UNITS")\


pivoted_channel_df.display()
...
pivoted_l3_promo_df = pivoted_l3_promo_df.filter(~(F.col('household_id') == -1))
...
pivoted_l6_promo_df = pivoted_l6_promo_df.filter(~(F.col('household_id') == -1))
...
pivoted_l9_promo_df = pivoted_l9_promo_df.filter(~(F.col('household_id') == -1))
# pivoted_l9_dep_df.display()
# pivoted_dep_df.count()
...
l3_promo_item_df = full_flag_df.filter(F.col('last_3_flag') == 'Y')\
                       .groupBy('household_id', 'promo_flag')\
                       .agg(countDistinct('upc_id').alias('Items'))
...
full_flag_df.filter(F.col('last_9_flag') == 'Y')\
                       .groupBy('household_id', 'promo_flag')\
                       .agg(countDistinct('upc_id').alias('Items')).display()

# COMMAND ----------

pivoted_l9_promo_item_df.filter((F.col('L9_PROMO_ITEMS') > 0) & (F.col('L9_MARK_DOWN_ITEMS') > 0)).display()
...
truprice_df = sqlContext.table('tdm_seg.srai_truprice_full_history').filter(F.col('period_id') == max_period_id)

#cc tenure
cc_tenure_df = full_flag_df.select('household_id', 'CC_TENURE').distinct()

# load table back (categories)
agg_df = spark.table("tdm_seg.kritawatkrai_th_sample_year_full_agg_data_jun_29_tmp")

more_agg_df = agg_df.join(weekly_df, on='household_id', how='left')\
                       ...
                       .join(truprice_df, on='household_id', how='left')\
...
full_agg_df.printSchema()

# COMMAND ----------

len(more_agg_df.columns)

# COMMAND ----------

len(full_agg_df.columns)

# COMMAND ----------

df_cols = full_agg_df.columns
...
full_agg_df.write.mode("overwrite").saveAsTable("tdm_seg.kritawatkrai_th_sample_year_full_agg_data_tmp")
ping()

# COMMAND ----------

column_names = full_agg_df.columns

invalid_characters = " ,;{}()\n\t="

def contains_invalid_characters(column_name):
    for char in invalid_characters:
        if char in column_name:
            return True
    return False

columns_with_invalid_characters = [col_name for col_name in column_names if contains_invalid_characters(col_name)]

if len(columns_with_invalid_characters) > 0:
    print("Columns with invalid characters:")
    for col_name in columns_with_invalid_characters:
        print(col_name)
else:
    print("No columns contain invalid characters.")



# COMMAND ----------

p = sqlContext.table('tdm.v_prod_dim_c').select(['upc_id','brand_name','division_id','division_name','department_id','department_name','department_code','section_id','section_name','section_code','class_id','class_name','class_code','subclass_id','subclass_name','subclass_code'])\
                                                 .filter(F.col('division_id').isin(product_division))\
                                                 .filter(F.col('country') == country)

p.filter(F.col('section_code') == '2_66_353').display()

# COMMAND ----------

p.filter(F.col('section_name') == 'WS SALAD').display()
